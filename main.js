...
const DATA_FILE = path.join(DATA_DIR, 'postit-wall.json');
...
// True when Windows/Electron relaunched us as a login item (see setStartWithWindows).
...
function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    skipTaskbar: false,
    show: !isAutoStarted,   // launched at login: stay hidden in the tray until opened
    title: 'Post-It Wall',
    icon: appIcon(),
    webPreferences: {
...
function setStartWithWindows(enabled) {
  ...
  const args = app.isPackaged ? ['--autostart'] : [__dirname, '--autostart'];
...
app.setAppUserModelId('com.postitwall.app');
