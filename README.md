...
A borderless, always-available desktop wall of freely arranged **sticky notes** and
...
**Windows:** download **`Stick-It Wall Setup <version>.exe`** from the
[latest release](https://github.com/HenrikPankoke/Stick-It-Wall/releases/latest) and run
it. It's an NSIS installer — you can choose the install location. The app then lives in
the **system tray** (yellow sticky-note icon).
...
```
%APPDATA%/stick-it-wall/stick-it-wall.json
```
...
runs **electron-builder**, producing `dist/Stick-It Wall Setup <version>.exe` (Windows
NSIS installer).
...
icon.js              Programmatic app icon (sticky note), rendered to PNG at build time
